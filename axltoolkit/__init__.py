...
class AxlToolkit:
    ...
    def list_users(self, **kwargs):
        ...
        if kwargs is not None:
            for key, value in kwargs.items(): #changed iteritems to items
                if key in allowed_tags:
                    search_criteria[key] = value

        if len(search_criteria) == 0:
            search_criteria['userid'] = '%'
            print("The search criteria is set to:", search_criteria)

        returned_tags = {'firstName': '', 'lastName': '', 'userid': '', 'homeCluster': '', 'imAndPresenceEnable': '', 'serviceProfile': ''}
